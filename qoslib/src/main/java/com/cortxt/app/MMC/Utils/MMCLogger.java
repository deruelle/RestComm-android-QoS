...
public class MMCLogger {
	...
	/**
	 * Gets the debug flag from the application context
	 * and stores it in {@link MMCLogger#DEBUGGABLE}
	 * @param app
	 */
	public static void initDebuggable (Context app) {
		MMCLogger.DEBUGGABLE = ( 0 != ( app.getApplicationInfo().flags &= ApplicationInfo.FLAG_DEBUGGABLE));
		String serverURL = app.getString(R.string.MMC_URL_LIN);
		if (serverURL.contains("dev."))
			MMCLogger.DEBUGGABLE = true;
	}
