...
public class EventResponse {
    ...
    public void handleEventResponse(MMCService owner, boolean commandsOnly)
    {
        if(this.getCommand() != null && this.getCommand().length() > 2) {
            Intent intent = new Intent(MMCIntentHandlerOld.COMMAND);
            String cmds = this.getCommand().toString ();
            intent.putExtra(MMCIntentHandlerOld.COMMAND_EXTRA, cmds);
            intent.putExtra("STARTTIME_EXTRA", d.starttime);
            owner.sendBroadcast(intent);
        }
        if (this.getSurvey() != null && this.getSurvey() > 0)
        {
            Intent intent = new Intent(MMCIntentHandlerOld.SURVEY);
            intent.putExtra(MMCIntentHandlerOld.SURVEY_EXTRA, this.getSurvey());
            owner.sendBroadcast(intent);
        }
        ...
        if (this.getSpeedTest() > 0)
        {
            ...
            intent.putExtra(SpeedTestTrigger.EXTRA_SPEED_TRIGGER, 1);
            intent.putExtra(SpeedTestTrigger.EXTRA_UPDATE_UI, false);
            owner.sendBroadcast(intent);
        }
