...
public class CoverageRequestWin  {
	...
	public CoverageRequestWin(Context context, String type, Carrier car, GeoPoint sw, GeoPoint ne, String mapmethod, int floor) {
		...
		String apiKey = MMCService.getApiKey(context); // PreferenceManager.getDefaultSharedPreferences(context).getString("API_KEY_PREFERENCE", null);
		
		paramsString += "&apiKey=" + apiKey;
		paramsString = paramsString.replace(" ", "%20");
		try {
			String request = context.getString(R.string.MMC_URL_LIN) + END_POINT_LIN + "?" + paramsString;
			url = new URL(request);
