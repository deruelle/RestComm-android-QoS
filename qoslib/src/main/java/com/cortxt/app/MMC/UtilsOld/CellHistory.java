	...
	public String updateNeighborHistory (int[] _list, int[] _list_rssi)
	{
		...
		//if (!_isTravelling)
		{
			...
			try
			{
				...
				int gen = MMCPhoneStateListenerOld.getNetworkGeneration(netType);
				int n = 0, len =0;
				boolean bValid = true;
				...
				try
				{
					...
					if (cells != null && cells.size() > 0 && cells.get(0) instanceof CellInfoLte && telephonyManager.getNetworkType() == MMCPhoneStateListenerOld.NETWORK_NEWTYPE_LTE)
					{
...
//		String hdr = "3,sec,psc,rssi, ";
	...
	public class CellidSample
	{
		...
		{
			MMCCellLocationOld mmcCell = new MMCCellLocationOld(cell);
