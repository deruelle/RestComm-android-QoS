...
import com.cortxt.app.MMC.Activities.AudioTest;
import com.cortxt.app.MMC.Activities.MMCActivity;
import com.cortxt.app.MMC.Activities.RawTest;
import com.cortxt.app.MMC.Activities.VideoTest;
import com.cortxt.app.MMC.Activities.WebPageTest;
import com.cortxt.app.MMC.Activities.YouTubeTest;
...
import com.cortxt.app.MMC.Activities.EngineeringModSettings;
import com.cortxt.app.MMC.Activities.EventHistory;
import com.cortxt.app.MMC.Activities.ShareTask;
import com.cortxt.app.MMC.Activities.VoiceQTest;
...
import com.cortxt.app.MMC.ActivitiesOld.CustomViews.SatelliteCount;
import com.cortxt.app.MMC.ActivitiesOld.CustomViews.SatelliteStatus;
...
//		registerReceiver(broadcastReceiver, intentFilter);
	...
	protected void onCreateOld(Bundle savedInstanceState) {
		...
		MMCActivity.customizeTitleBar(this, view, R.string.dashboard_engineer, R.string.dashcustom_engineer);
		LinearLayout chartLayout = (LinearLayout) findViewById(R.id.chartLayout);
		RelativeLayout satellitelayout = (RelativeLayout) findViewById(R.id.satellitelayout);
...
/*			int grayColor = Integer.parseInt(screenColor, 16);
	...
	protected void onResume() {
		...
		if (MMCService.getGpsManager() != null && MMCService.getGpsManager().isGpsRunning())
	...
	public static void initEngg (Context context, View enggView, NerdScreen enggActivity)
	{
		...
		MMCActivity.customizeTitleBar(context, enggView, R.string.dashboard_engineer, R.string.dashcustom_engineer);
...
//			backLogo.setOnClickListener(enggpanelAct.BackPanelClicked);
		...
		if (systemAlertPermissionValue == 0) {
			...
			b.setOnCancelListener(new DialogInterface.OnCancelListener() {
				...
				public void onCancel(DialogInterface dialog) {
					svcPanel.dismiss();
					svcPanel = null;
					MMCPhoneStateListenerOld.closedServicePanel = true;
				}
			});
		...
		if (!MMCService.isInTracking()) {
			...
			alertDialogBuilder.setPositiveButton(R.string.GenericText_Yes, new OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					...
					recordingEndsIn.setVisibility(View.INVISIBLE);
					Intent intent = new Intent(MMCIntentHandlerOld.STOP_TRACKING_ACTION);
	...
	/*	@Override
		public boolean onOptionsItemSelected(MenuItem item) {
			...
			//case R.id.Livestatus_Menu_Update:
			//start gps
			//	intentDispatcher.triggerUpdateEvent();
			//	return true;
			case R.id.Livestatus_Menu_ColdReset:
				// cold reset the gps
				intentDispatcher.gpsColdReset();
				return true;

				//case R.id.Livestatus_Menu_OemRil:
				//	Intent startOemRilIntent = new Intent(this,
				//			RilOemHookTest.class);
				//	startActivity(startOemRilIntent);
				//	return true;

			case R.id.Livestatus_Menu_Track:
				//return handleStartTracking (this, item);
				/*if (!MMCService.isInTracking())
				{
	...
	private void resumeRecording() {
		if (MMCService.isInTracking()) {
			...
			if (MMCService.isInTracking()) // diff>0){
	 ...
	 * {@value TablesOld.SignalStrengths.CDMAVersion#RSSI} or the
	 * {@value TablesOld.SignalStrengths.GSMVersion#SIGNAL_STRENGTH} column.
	 * 
	 * @param cursor
	 *            The cursor that was probably generated in the {@link #updateActivityFromDB()} method.
	 */
	private void updateNerdViewFromDB() {
		...
		if (nerdview == null)
			return;
		try {
			Uri signalUri = UriMatchOld.SIGNAL_STRENGTHS.getContentUri();
			Uri limitSignalUri = signalUri.buildUpon().appendQueryParameter("limit", "1").build();
			// sig_cursor = managedQuery(

			ProviderOld dbProvider = ReportManager.getInstance(context.getApplicationContext()).getDBProvider();
			if (dbProvider == null) {
				return;
			}

			sig_cursor = dbProvider.query(UriMatchOld.SIGNAL_STRENGTHS.getContentUri(), null, // new String[]{
																								// TablesOld.TIMESTAMP_COLUMN_NAME,
																								// TablesOld.SignalStrengths.SIGNAL,
																								// TablesOld.SignalStrengths.LTE_RSRP
																								// },
					null, // TablesOld.TIMESTAMP_COLUMN_NAME + ">?",
			...
			Uri baseStationTable = (UriMatchOld.BASE_STATIONS.getContentUri()/*telephonyManager.getPhoneType() == TelephonyManager.PHONE_TYPE_CDMA 
			...
			cell_cursor = dbProvider.query(limitBSUri, null, null, null, TablesOld.TIMESTAMP_COLUMN_NAME + " DESC");

			cell_cursor.moveToFirst();

			int LowIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.BS_LOW);
			int MidIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.BS_MID);
			int HighIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.BS_HIGH);
			int CodeIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.BS_CODE);
			int BandIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.BS_BAND);
			int ChanIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.BS_CHAN);
			int netTypeIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.NET_TYPE);
			...
			if (sig_cursor.getCount() != 0) {
				int signalIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.SIGNAL);
				int signal2GIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.SIGNAL2G);
				int rsrpIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.LTE_RSRP);
				int rsrqIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.LTE_RSRQ);
				int lteSnrIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.LTE_SNR);
				int lteSignalIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.LTE_SIGNAL);
				int lteCqiIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.LTE_CQI);
				int ecioIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.ECI0);
				int ecnoIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.ECN0);
				int snrIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.SNR);
				int berIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.BER);
				int rscpIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.RSCP);
				int tierIndex = sig_cursor.getColumnIndex(TablesOld.SignalStrengths.COVERAGE);
...
//							nerdview.setValue(1, "Channel", null);
			...
			// int BIDIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.CDMAVersion.BID); //getlowmid high
			// if(BIDIndex != -1)
			// nerdview.setValue(1, "BID",Integer.toString(cell_cursor.getInt(BIDIndex)));
			//
			// //set the network id
			// int NIDIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.CDMAVersion.NID);
			// if (NIDIndex != -1)
			// nerdview.setValue(1, "NID",Integer.toString(cell_cursor.getInt(NIDIndex)));
			//
			// //set the system id
			// int SIDIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.CDMAVersion.SID);
			// if (SIDIndex != -1)
			...
			// int cellidIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.GSMVersion.CELL_ID);
			//
			// if(cellidIndex != -1)
			// {
			...
			// //the network Id is kept 0 for gsm phones
			// //set the lac
			// int lacIndex = cell_cursor.getColumnIndex(TablesOld.BaseStations.GSMVersion.LAC);
			// if (lacIndex != -1)
	...
	private BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (intent.getAction().equals(CommonIntentBundleKeysOld.ACTION_UPDATE)) {
				...
				manageConnectUpdate();
			}else if (intent.getAction().equals(MMCIntentHandlerOld.ACTION_DONE_SMSTEST)) {
	...
	class IntentDispatcher {
		...
		public void triggerUpdateEvent() {
			Intent intent = new Intent(MMCIntentHandlerOld.UPDATE_ACTION);
			sendBroadcast(intent);
		}
        public void startWebSocket(boolean isRunning) {

            Intent intent = new Intent(MMCIntentHandlerOld.RUN_WEBSOCKET);
            intent.putExtra(MMCIntentHandlerOld.EXTRA_START_WEBSOCKET, !isRunning);
            sendBroadcast(intent);
        }

		public void gpsColdReset() {
			Intent intent = new Intent(MMCIntentHandlerOld.COLDRESET_ACTION);
			sendBroadcast(intent);
		}
		...
		public void startTracking(int numFiveMinutePeriodsToTrack) {
			Intent intent = new Intent(MMCIntentHandlerOld.START_TRACKING_ACTION);
			intent.putExtra(MMCIntentHandlerOld.TRACKING_NUM_5_MIN_PERIODS, numFiveMinutePeriodsToTrack);
			sendBroadcast(intent);
		}
	...
	public void menuActionClicked(View menuButton) {
		...
		if (currentTime - DropDownMenuWindow.lastWindowDismissedTime > 200) {
			...
			TextView rawTestOption=(TextView)menuOptionsView.findViewById(R.id.RawTestOption);
			...
			FontsUtil.applyFontToTextView(MmcConstants.font_Regular, rawTestOption, this);
			...
			rawTestOption.setOnClickListener(rawTestClickListener);
			...
			if (MMCService.getSMSPermissionsAllowed(this, true) == true ){
			...
			{
				// Web Test disabled
				rawTestOption.setVisibility(View.GONE);
				menuOptionsView.findViewById(R.id.rawSeparator).setVisibility(View.GONE);
...
//            }
	...
	android.view.View.OnClickListener smsTestStartClickListener = new android.view.View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			Intent intent = new Intent(MMCIntentHandlerOld.SMS_TEST);
	...
	android.view.View.OnClickListener latencyTestClickListener = new android.view.View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			Intent intent = new Intent(MMCIntentHandlerOld.LATENCY_TEST);
	...
	android.view.View.OnClickListener rawTestClickListener=new android.view.View.OnClickListener () {

		@Override
		public void onClick(View v) {
			Intent intent = new Intent(NerdScreen.this, RawTest.class);
			startActivity(intent);
			if(engineerMenu!=null){
				engineerMenu.dismissWindow();
			}
