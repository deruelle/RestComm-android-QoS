		...
		if (getIntent().hasExtra(EXTRA_EVENT_ID)) {
			...
			mGetEventInfoTask = new AsyncTask<Void, Void, HashMap<String, String>>() {
				...
				private void showEventInfo() {
					long timeStamp = Long.parseLong(mEvent.get(EventKeys.TIMESTAMP));
				...
				private void loadMap() {
					try {
						if (mEvent.containsKey(EventKeys.LATITUDE) && mEvent.containsKey(EventKeys.LONGITUDE)) {
							int latitudeE6 = (int) (Double.parseDouble(mEvent.get(EventKeys.LATITUDE)) * 1000000.0);
							int longitudeE6 = (int) (Double.parseDouble(mEvent.get(EventKeys.LONGITUDE)) * 1000000.0);
						...
						Integer itype = null;
						if (mEvent.get(EventKeys.TYPE) != null)
							itype = Integer.parseInt(mEvent.get(EventKeys.TYPE));
						if (itype == null)
							return;
						...
						if (eventType == EventType.EVT_DROP || eventType == EventType.EVT_CALLFAIL || eventType == EventType.EVT_DISCONNECT || eventType == EventType.EVT_UNANSWERED) {
							int rating = Integer.parseInt(mEvent.get(EventKeys.RATING));
				...
				private void getAddress(final HashMap<String, String> event) {
					if (event.containsKey(EventKeys.LATITUDE) && event.containsKey(EventKeys.LONGITUDE)) {
						...
						Runnable r = new Runnable() {
							...
							@Override
							public void run() {
								String addressString = WebReporter.geocode(Double.parseDouble(event.get(EventKeys.LATITUDE)), Double.parseDouble(event.get(EventKeys.LONGITUDE)));
...
//								}
						...
						new Thread(r).start();
						try{
						String address = String.format("%.4f, %.4f", Double.valueOf(event.get(EventKeys.LATITUDE)), Double.valueOf(event.get(EventKeys.LONGITUDE)));
				...
				protected void onPostExecute(final HashMap<String, String> event) {
					...
					if (event == null)
						return;
					long timeStamp = Long.parseLong(event.get(EventKeys.TIMESTAMP));
...
//	@Override
	...
	private void confirmDropEvent(boolean bConfirm) {
		final int evtID = Integer.parseInt(mEvent.get(EventKeys.ID));
		final EventType evtType = EventType.get(Integer.parseInt(mEvent.get(EventKeys.TYPE)));

		final EventType newEvtType;
		final int rating;
		final long ltime = Long.parseLong(mEvent.get(EventKeys.TIMESTAMP));
		if (bConfirm)
			rating = 6;
		else
			rating = 0;
		...
		if (evtType == EventType.EVT_DISCONNECT && bConfirm)
			...
			newEvtType = evtType;
		final int iUserID = MMCService.getUserID(this);
		final ReportManager reportManager = ReportManager.getInstance(getApplicationContext());
		reportManager.updateEventField(evtID, Events.KEY_TIER, Integer.toString(rating));
		if (newEvtType != evtType) {
			reportManager.updateEventField(evtID, Events.KEY_TYPE, Integer.toString(newEvtType.getIntValue()));
		}
	...
	private static void showMapIcon(int imageResource, boolean animate) {
		if (!mEvent.containsKey(EventKeys.LATITUDE)) {
	...
	{
		int eventType = Integer.parseInt(event.get(EventKeys.TYPE));
		String details = "";
	...
	public static String getTier(HashMap<String, String> event) {
		int tier = Integer.parseInt(event.get(EventKeys.RATING));
	...
	public void shareClicked(View button) {
		...
		mHandler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				EventType eventType = EventType.get(Integer.parseInt(mEvent.get(EventKeys.TYPE)));
