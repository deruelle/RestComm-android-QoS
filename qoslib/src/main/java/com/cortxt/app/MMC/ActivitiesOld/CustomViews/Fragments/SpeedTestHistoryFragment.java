...
import com.cortxt.app.MMC.Activities.SpeedTestHistory;
...
public class SpeedTestHistoryFragment extends ListFragment {
	...
	private List<HashMap<String, String>> getFormattedResults(List<HashMap<String, Long>> rawResults) {
		...
		for (HashMap<String, Long> rawResult : rawResults) {
			...
			long tier = rawResult.get(SpeedTestKeys.SPEEDTIER);
			result.put(SpeedTestKeys.SPEEDTIER, Long.toString(tier));

			long latency = rawResult.get(SpeedTestKeys.LATENCY);
			result.put(SpeedTestKeys.LATENCY, latency + " " + getString(R.string.speedtest_milliseconds));

			long downSpeed = rawResult.get(SpeedTestKeys.DOWNLOAD_SPEED);
			result.put(SpeedTestKeys.DOWNLOAD_SPEED, getFormattedSpeed((int) downSpeed) + " " + getUnits((int) downSpeed));

			Long upSpeed = (rawResult.get(SpeedTestKeys.UPLOAD_SPEED));
			if (eventtype != EventType.MAN_SPEEDTEST.getIntValue())
				result.put(SpeedTestKeys.UPLOAD_SPEED, upSpeed.toString());
			else
				result.put(SpeedTestKeys.UPLOAD_SPEED, getFormattedSpeed((int)(long) upSpeed) + " " + getUnits((int)(long) upSpeed));

			long type = rawResult.get(EventKeys.TYPE);
			result.put(EventKeys.TYPE, Long.toString(type));

			long timeStamp = rawResult.get(SpeedTestKeys.TIMESTAMP);
			// long timeStamp =
			...
			result.put(SpeedTestKeys.TIMESTAMP, date + "\n" + time);
	...
	private class SpeedHistoryAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			String latency = historyResults.get(position).get(SpeedTestKeys.LATENCY);
			latencySpeed.setText("" + latency);
			String speed = historyResults.get(position).get(SpeedTestKeys.DOWNLOAD_SPEED);
			String type =  historyResults.get(position).get(EventKeys.TYPE);
			if (type.equals("54"))
			{
			...
			else
			{
				...
				speed = historyResults.get(position).get(SpeedTestKeys.UPLOAD_SPEED);
				if (speed.equals("0.0 Kb/s"))
					speed = "";
			...
			speedDate.setText("" + historyResults.get(position).get(SpeedTestKeys.TIMESTAMP));

			int tier = Integer.parseInt(historyResults.get(position).get(SpeedTestKeys.SPEEDTIER));
			switch (tier) {
			case 2:
