...
//		webView.loadUrl(mTestUrl);
    ...
    {
        Intent intent = new Intent(MMCIntentHandlerOld.ACTIVE_TEST);
        intent.putExtra(SpeedTestTrigger.EXTRA_SPEED_TRIGGER, 0);
        intent.putExtra(SpeedTestTrigger.EXTRA_TEST_TYPE, EventType.YOUTUBE_TEST.getIntValue());
		VideoTestTrigger.setWebView(webView);
...
//		}
    ...
    {
        if(intent.hasExtra(MMCIntentHandlerOld.EXTRA_BUFFER_PROGRESS)) {
            int bufferProgress = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_BUFFER_PROGRESS, 0);
            int playProgress = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_PLAY_PROGRESS, 0);
            int stallCount = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_STALLS, 0);
            int stallTime = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_STALL_TIME, 0);
            int duration = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_DURATION, 0);
            int accessDelay = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_ACCESS_DELAY, 0);
            int playDelay = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_PLAY_DELAY, 0);
			int downloadTime = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_DOWNLOAD_TIME, 0);
            int eventtype = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_EVENTTYPE, 0);
			int rxbytes = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_RXBYTES, 0);
			mheight = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_VIDEO_RES, 0);
