...
import com.cortxt.app.MMC.Activities.SatisfactionSurvey;
 ...
 *
 */
public class MMCIntentHandlerOld extends BroadcastReceiver {
	public static final String TAG = MMCIntentHandlerOld.class.getSimpleName();
	...
	*
	...
	//private variables
	private static MMCService owner;	//keep a local reference to the owner class in order to access the important variables
    private DataMonitorStats dataMonitorStats;
    private static EventOld manualPlottingEvent = null;
    private static EventOld manualTransitEvent = null;
    private WakeLock wakeLock = null;
    private Gson gson;
			
	//constants related to the intents MMCService can receive
	/**
...
//	public static final String ACTION_NETWORK_LOCATION_UPDATE = "com.cortxt.app.MMC.intent.action.ACTION_NETWORK_LOCATION_UPDATE";
	 ...
	 * constructor
	 */
	public MMCIntentHandlerOld(MMCService owner, DataMonitorStats dataMonitorStats){
		this.owner = owner;	
	...
	public MMCIntentHandlerOld(){

	}
	
	public static EventOld getPlottingEvent() {
		return manualPlottingEvent;
	}
	
	public static EventOld getTransitEvent() {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(Intent.ACTION_BATTERY_CHANGED)){
            ...
            int trigger = intent.getIntExtra(SpeedTestTrigger.EXTRA_SPEED_TRIGGER, 0);
			...
			//this is supposed to trigger a speed test
            int evType = intent.getIntExtra (SpeedTestTrigger.EXTRA_TEST_TYPE, 0);
            ...
            int testType = intent.getIntExtra(SpeedTestTrigger.EXTRA_TEST_TYPE, 0);
		...
		else if (action.equals("android.intent.action.ACTION_DATA_CONNECTION_FAILED"))
		{
			...
			dataMonitorStats.setWifi(owner.isNetworkWifi());
		}
...
//			owner.trackAccessPoints();
			...
			try {
				...
				MMCLogger.logToFile(MMCLogger.Level.ERROR, TAG, "onReceive", "error receiving Intent.ACTION_HEADSET_PLUG: " + e);
			}
		}
		else if(action.equals(Intent.ACTION_HEADSET_PLUG)) {
			int state = intent.getIntExtra("state", -1);
		...
		else if (action.equals(VQManager.ACTION_START_VOICETEST)){
			//this is supposed to trigger a speed test
			int trigger = intent.getIntExtra(VQManager.EXTRA_VQ_TRIGGER, 0);
			owner.getVQManager().runTest(trigger);
		}
		else if (action.equals(VQManager.ACTION_TEST_VQ_DEVICE)){
			owner.getVQManager().runTest(10);
		}
		else if (action.equals(VQManager.ACTION_STOP_VOICETEST)){
			...
			try {
				...
				{
					owner.getTravelDetector().screenChanged(false);
					owner.getEventManager().screenChanged(false);
				}
			} catch (Exception e) {
				MMCLogger.logToFile(MMCLogger.Level.ERROR, TAG, "onReceive", "received action SCREEN_OFF, calling MMCService.processNewMMCSignal()", e);
			}
		}
			...
			if (owner.getTravelDetector() != null)
			{			
				owner.getTravelDetector().screenChanged(true);
            ...
            MMCLogger.logToFile(MMCLogger.Level.DEBUG, TAG, "onReceive", "ACTION_TRACKING_1MINUTE");
...
//		}
			...
			if (intervalDM > 0)
			{
				...
				if (!firstBucketDone) { //if false, first bucket needs to be done
					...
					MMCLogger.logToFile(MMCLogger.Level.DEBUG, "MMCIntentHandlerOld", "onReceive", "First bucket at: " + System.currentTimeMillis()/1000);
				}
				else {
					dataMonitorStats.monitor();
...
//    				PreferenceKeys.Miscellaneous.SURVEY_COMMAND,).commit();
			...
			manualTransitEvent = owner.getEventManager().triggerSingletonEvent(EventType.MAN_TRANSIT);
			manualTransitEvent.setLocation(location, 0);
		}
		else if(action.equals(MANUAL_TRANSIT_END)) {
			if(manualTransitEvent == null)
				return;
			...
			if (corrected != 0)
			{
				Location location = manualTransitEvent.getLocation();
				location.setAccuracy(-4);
			}
			
			manualTransitEvent.setAppData(accelData); //TODO want this to really be in appdata?
			manualTransitEvent.setLookupid1(stationFrom);
			manualTransitEvent.setLookupid2(stationTo);
			manualTransitEvent.setDuration(duration);
			owner.getEventManager().unstageAndUploadEvent(manualTransitEvent, null);
			manualTransitEvent = null;
		}
		else if(action.equals(MANUAL_TRANSIT_CANCEL)) {
			if(manualTransitEvent != null) {
				owner.getEventManager().unstageEvent(manualTransitEvent);
				ReportManager reportManager = ReportManager.getInstance(owner);
				reportManager.getDBProvider().delete(TablesEnumOld.LOCATIONS.getContentUri(), "timestamp > ? And accuracy = 3", new String[]{ String.valueOf(manualTransitEvent.getEventTimestamp())});	
				manualTransitEvent = null;
			}
		}
...
//			int type = intent.getIntExtra("type", -1);  //1(indoor) or 2(outdoor)
			...
			manualPlottingEvent = owner.getEventManager().triggerSingletonEvent(EventType.MAN_PLOTTING);
			manualPlottingEvent.setEventIndex(floor);
			manualPlottingEvent.setDuration(topFloor);
			manualPlottingEvent.setBuildingID (osm_id);
			manualPlottingEvent.setAppData(poly);
			Location location = new Location("");	
			location.setLatitude(lat/1000000.0);
			location.setLongitude(lng/1000000.0);
			location.setAccuracy(-1);
			
			manualPlottingEvent.setLocation(location, 0);
		}
		else if(action.equals(MANUAL_PLOTTING_END)) {
//			owner.getEventManager().unstageEvent(manulaPlottingEvent); //does not upload
			if(manualPlottingEvent != null)
				owner.getEventManager().unstageAndUploadEvent(manualPlottingEvent, null);
			//After the event was submitted, reset it so we don't restore an old event in ManualMapping
			manualPlottingEvent = null;
		}
		else if(action.equals(MANUAL_PLOTTING_CANCEL)) {
			if(manualPlottingEvent != null)
			{
				owner.getEventManager().unstageEvent(manualPlottingEvent);
				ReportManager reportManager = ReportManager.getInstance(owner);
				
				reportManager.getDBProvider().delete(TablesEnumOld.LOCATIONS.getContentUri(), "timestamp > ? And accuracy < 0", new String[]{ String.valueOf(manualPlottingEvent.getEventTimestamp())});	
				manualPlottingEvent = null;
...
//			String msg_from;
	...
	//            msg_from = msgs[i].getOriginatingAddress();
            ...
            {
                MMCLogger.logToFile(MMCLogger.Level.ERROR, TAG, "onReceived GCM_MESSAGE", "exception", e);
            }
        }
		...
		else if (intent.getAction().equals("android.intent.action.PRECISE_CALL_STATE"))
		{
			...
			MMCLogger.logToFile(MMCLogger.Level.ERROR, TAG, "PRECISE_CALL_STATE", precisecall.toString());
...
//			}
		...
		else if (intent.getAction().equals(ACTION_RADIOLOG_ERROR))
		{
			...
			MMCLogger.logToFile(MMCLogger.Level.ERROR, "RilReader", error, details);
			//owner.onConnect(time, state);
		}
		...
		try {
			for(int i = 0; i < msgs.length; i++) {
	    		...
	    		if(msgBody.indexOf("\"mmctype\":") != -1) {
                    MMCLogger.logToFile(MMCLogger.Level.DEBUG, TAG, "handleCommands", msgBody);
	    			if (sms)
	    				abortBroadcast();
					...
					if(msgBody.indexOf("\"schedule\":") != -1) {
                        ...
                        MMCLogger.logToFile(MMCLogger.Level.ERROR, TAG, "handleCommands", "triggerSpeedTest");
                        ...
                        MMCLogger.logToFile(MMCLogger.Level.DEBUG, TAG, "runTracking", "beginVoiceTest");
	    	...
	    	MMCLogger.logToFile(MMCLogger.Level.ERROR, TAG, "handleCommands", "exception", e);
	    }	
	}

	public IntentFilter declareIntentFilters() {
		IntentFilter intentFilter = new IntentFilter(MMCIntentHandlerOld.UPDATE_ACTION);
		intentFilter.addAction(MMCIntentHandlerOld.START_TRACKING_ACTION);
		intentFilter.addAction(MMCIntentHandlerOld.TRACK_REMOTE);
		intentFilter.addAction(MMCIntentHandlerOld.STOP_TRACKING_ACTION);
		intentFilter.addAction(MMCIntentHandlerOld.SPEED_TEST);
		intentFilter.addAction(MMCIntentHandlerOld.RUN_WEBSOCKET);
		...
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_SPEEDTEST_RESULT);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_SPEEDTEST_ERROR);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_SPEEDTEST_COMPLETE);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_SPEEDTEST_CANCELLED);

		intentFilter.addAction(MMCIntentHandlerOld.ACTION_WEBTEST_RESULT);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_WEBTEST_ERROR);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_WEBTEST_COMPLETE);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_WEBTEST_CANCELLED);

		//do not add filter if sms test permissions aren't all allowed
		if (owner.getSMSPermissionsAllowed(owner, true))
			intentFilter.addAction(MMCIntentHandlerOld.SMS_TEST);

		intentFilter.addAction(MMCIntentHandlerOld.SMS_DELIVERED);
		intentFilter.addAction(MMCIntentHandlerOld.LATENCY_TEST);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_STOP_SPEEDTEST);
		intentFilter.addAction(MMCIntentHandlerOld.RESTART_MMC_SERVICE);
		intentFilter.addAction(MMCIntentHandlerOld.COLDRESET_ACTION);
		intentFilter.addAction(Intent.ACTION_BATTERY_CHANGED);
		intentFilter.addAction(Intent.ACTION_POWER_CONNECTED);
		intentFilter.addAction(Intent.ACTION_POWER_DISCONNECTED);
		intentFilter.addAction(Intent.ACTION_SCREEN_OFF);
		intentFilter.addAction(Intent.ACTION_SCREEN_ON);
		intentFilter.addAction(MMCIntentHandlerOld.COMMAND);
		intentFilter.addAction(MMCIntentHandlerOld.SURVEY);
		//intentFilter.addAction(Intent.ACTION_VIEW);
		//intentFilter.addAction("android.intent.PHONE_STATE");
		intentFilter.addAction(WifiManager.NETWORK_STATE_CHANGED_ACTION);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_ALARM_MINUTE);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_TRACKING_5MINUTE);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_TRACKING_1MINUTE);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_ALARM_3HOUR);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_ALARM_15MINUTE);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_ALARM_SCANAPPS);
		intentFilter.addAction(MMCIntentHandlerOld.EMAIL_CSV);
		intentFilter.addAction(MMCIntentHandlerOld.GPS_STATE_OFF);
		intentFilter.addAction(MMCIntentHandlerOld.GPS_STATE_ON);
		intentFilter.addAction(MMCIntentHandlerOld.PHONE_CALL_CONNECT);
		intentFilter.addAction(MMCIntentHandlerOld.PHONE_CALL_DISCONNECT);
		intentFilter.addAction(MMCIntentHandlerOld.HANDOFF);
		intentFilter.addAction(MMCIntentHandlerOld.SMS_SENT);
		intentFilter.addAction(MMCIntentHandlerOld.SMS_RECEIVED);
		intentFilter.addAction(Intent.ACTION_SEND);
		intentFilter.addAction(MMCIntentHandlerOld.MANUAL_PLOTTING_START);
		intentFilter.addAction(MMCIntentHandlerOld.MANUAL_PLOTTING_END);
		intentFilter.addAction(MMCIntentHandlerOld.MANUAL_PLOTTING_CANCEL);
		intentFilter.addAction(MMCIntentHandlerOld.MANUAL_TRANSIT_START);
		intentFilter.addAction(MMCIntentHandlerOld.MANUAL_TRANSIT_END);
		intentFilter.addAction(MMCIntentHandlerOld.MANUAL_TRANSIT_CANCEL);
		intentFilter.addAction(BluetoothDevice.ACTION_ACL_CONNECTED);
		intentFilter.addAction(BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED);
		intentFilter.addAction(BluetoothDevice.ACTION_ACL_DISCONNECTED);

		intentFilter.addAction(Intent.ACTION_HEADSET_PLUG);
		intentFilter.addAction(MMCIntentHandlerOld.ROAMING_ON);
		intentFilter.addAction(MMCIntentHandlerOld.ROAMING_OFF);
//		intentFilter.addAction(MMCIntentHandlerOld.ACTION_BRIEFLY_RUN_LOCATION);
//		intentFilter.addAction(MMCIntentHandlerOld.ACTION_GPS_LOCATION_UPDATE);
//		intentFilter.addAction(MMCIntentHandlerOld.ACTION_NETWORK_LOCATION_UPDATE);

		intentFilter.addAction(MMCIntentHandlerOld.VIEWING_SIGNAL);
...
//		intentFilter.addAction("com.android.internal.telephony.data-reconnect");
//		intentFilter.addAction("com.android.internal.telephony.dataconnectiontracker.intent_set_fail_data_setup_counter");

		intentFilter.addAction(VQManager.ACTION_BLUETOOTH_ENDDOWNLOAD);
		intentFilter.addAction(VQManager.ACTION_START_VOICETEST);
		intentFilter.addAction(VQManager.ACTION_STOP_VOICETEST);
		intentFilter.addAction(VQManager.ACTION_TEST_VQ_DEVICE);

		intentFilter.addAction(MMCIntentHandlerOld.ACTIVE_TEST);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_STOP_VIDEOTEST);
		intentFilter.addAction(MMCIntentHandlerOld.GCM_MESSAGE);
		...
		intentFilter.addAction (MMCIntentHandlerOld.ACTION_RADIOLOG_DISCONNECT);
		intentFilter.addAction (MMCIntentHandlerOld.ACTION_RADIOLOG_CONNECT);
		intentFilter.addAction (MMCIntentHandlerOld.ACTION_RADIOLOG_NEIGHBORS);
		intentFilter.addAction (MMCIntentHandlerOld.ACTION_RADIOLOG_SERVICEMODE);
		intentFilter.addAction (MMCIntentHandlerOld.ACTION_RADIOLOG_SERVICEMENU);
		intentFilter.addAction (MMCIntentHandlerOld.ACTION_MMCSYS_VERSION);
	...
	private void postSurvey (int surveyid)
	{
		...
		Intent notificationIntent = new Intent(owner, SatisfactionSurvey.class);
