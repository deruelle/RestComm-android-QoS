...
public class GpsManagerOld implements GpsStatus.Listener, LocationListener {
	 ...
	 */
	private ArrayList<GpsListenerOld> listeners = new ArrayList<GpsListenerOld>();
	 ...
	 * many listeners and start the operationTimeout for them. We also use it to show messages on the
	 * {@link NerdScreen} screen.
	 */
	@Override
	public void onGpsStatusChanged(int event) {
				...
				try{
					...
					if (lastloc == null || !lastloc.getProvider().equals(LocationManager.GPS_PROVIDER)) //  || lastloc.getTime() < System.currentTimeMillis() - 10000 )
					{
						for (int i=0; i<listeners.size(); i++) {
							GpsListenerOld listener = listeners.get(i);
		...
		case GpsStatus.GPS_EVENT_STARTED:
			...
			Intent intent = new Intent(MMCIntentHandlerOld.GPS_STATE_ON);
			owner.sendBroadcast(intent);
			break;
		case GpsStatus.GPS_EVENT_STOPPED:
			owner.updateNumberOfSatellites(0, 0); 
			owner.broadcastGpsStatus(false);
			Intent intent2 = new Intent(MMCIntentHandlerOld.GPS_STATE_OFF);
...
//			}
			...
			for (int i=0; i<listeners.size(); i++) {
				...
				boolean isAnotherLocationUpdateNeeded = listener.onLocationUpdate(location);
				
				// If location is accurate enough, consider this the first fix - this listener has new timeout
				if (location.getAccuracy() < GpsManagerOld.LOCATION_UPDATE_MIN_TREND_ACCURACY && listener.getOperationTimeout() > 0){
	...
	public void startupGpsFinished (GpsListenerOld listener)
	{
	...
	class FirstFixTimeoutTimerTask extends TimerTask {
		private GpsListenerOld listener;
		
		public FirstFixTimeoutTimerTask(GpsListenerOld listener){
	...
	class OperationTimeoutTimerTask2 extends TimerTask {
		...
		public OperationTimeoutTimerTask2(GpsListenerOld listener){
			this.listener = listener;
		}
		/**
	...
	public void safeguardGps ()
	{
		...
		synchronized(GpsManagerOld.this) {
			if (listeners.size() >= 1)
			{
				...
				{
					if (listeners.get(i) != null && listeners.get(i) instanceof EventManagerOld.GpsListenerForEvent)
					{
						EventManagerOld.GpsListenerForEvent evtlistener = (EventManagerOld.GpsListenerForEvent)listeners.get(i);
						EventOld thisEvent = evtlistener.thisEvent;
				
						if (thisEvent.getEventTimestamp() + 10*60000 < System.currentTimeMillis() && (thisEvent.getEventType() != EventType.EVT_CONNECT || !owner.getPhoneStateListener().isCallConnected()))
						{
	 ...
	 * @param listener
	 */
	public void registerListener(final GpsListenerOld listener){
		synchronized(GpsManagerOld.this) {
	 ...
	 * @param listener
	 */
	public void unregisterListener(GpsListenerOld listener){
		synchronized(GpsManagerOld.this) {
	 ...
	 */
	private void addListenerToCollection(GpsListenerOld listener){
		synchronized(GpsManagerOld.this) {
	 ...
	 */
	private void removeListenerFromCollection(GpsListenerOld listener){
		synchronized(GpsManagerOld.this) {
