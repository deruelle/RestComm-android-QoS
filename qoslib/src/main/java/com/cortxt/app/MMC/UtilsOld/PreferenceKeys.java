...
/**
 * The application routinely uses the preference framework offered by Android to 
 * persist key/value pairs. Some of these preferences are not available to the
 * user in the preference screen. These arbitrary key/value pairs can get their
 * key values from this file containing the constants.
 * 
 * TODO The things that the preference framework is being used to persist are
 * ideally supposed to be persisted in the database (possibly enveloped by a content
 * provider). Therefore, this class would have to be removed in the future. The preference
 * framework should only be used to store actual preferences and not flags as is being
 * done here.
 * @author abhin
 *
 */
