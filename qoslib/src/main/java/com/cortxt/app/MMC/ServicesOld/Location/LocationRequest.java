...
//	}
	...
	public LocationRequest (Context context, int gpsAccuracy) {
		...
		location = MMCService.getLastLocation();//ReportManager.getInstance(context.getApplicationContext()).getLastKnownLocation();
	...
	public void stop ()
	{
		if (locListener != null && MMCService.getGpsManager() != null)
			MMCService.getGpsManager().unregisterListener(locListener);
		if (locListener2 != null && MMCService.getNetLocationManager() != null)
			MMCService.getNetLocationManager().unregisterListener(locListener2);
		bGPSRunning = false;
		bNWRunning = true;
	}
	
	class GpsListenerForRequest extends GpsListenerOld {
		//
		...
		public boolean onLocationUpdate(Location location) {
			if (MMCService.getGpsManager() != null)
				gpsSatellites = MMCService.getGpsManager().getNumberOfSatellites();
			...
			{
				MMCService.processNewFilteredLocation(location, gpsSatellites, mContext);
	 ...
	 */
	class LocationListenerForRequest extends GpsListenerOld {
	//
