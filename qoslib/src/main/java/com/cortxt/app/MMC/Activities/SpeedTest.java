...
import com.cortxt.app.MMC.ActivitiesOld.CustomViews.Fragments.SpeedTestHistoryFragment;
...
//			mUploadSpeedValue.setLayoutParams(linearParams3);
		...
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, mLatency, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, mDownSpeed, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, mUpSpeed, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_MEDIUM, mCarrierAvg, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, mCarrierLatency, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, mCarrierDownSpeed, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, mCarrierUpSpeed, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_MEDIUM, mGaugeDownSpeed, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_MEDIUM, mGaugeDownUnits, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_MEDIUM, mGaugeUpSpeed, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_MEDIUM, mGaugeUpUnits, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_MEDIUM, resultHeading, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_MEDIUM, yourDevice, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, latencyText, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, downloadText, this);
		FontsUtil.applyFontToTextView(MmcConstants.font_Regular, uploadText, this);
...
//		}
		...
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_SIGNAL_STRENGTH_UPDATE);
		intentFilter.addAction(MMCIntentHandlerOld.ACTION_CONNECTION_UPDATE);
		...
		public void onClick(View v) {			
			...
			builder.setPositiveButton(getString(R.string.GenericText_Yes), new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {					
					Intent intent = new Intent(MMCIntentHandlerOld.ACTION_STOP_SPEEDTEST);		
...
//	}
	...
	public void startClicked(View button) {
			...
			builder.setPositiveButton(getString(R.string.GenericText_Yes), new DialogInterface.OnClickListener() {
			  ...
			  public void onClick(DialogInterface dialog, int which) {
				// launch settings screen
				  Intent bgServiceIntent = new Intent(SpeedTest.this, MMCService.class);
				  startService(bgServiceIntent);
				  trackEvent ("StartStop", "start", "SpeedTest", 0);
                  SecurePreferences securePrefs = MMCService.getSecurePreferences (SpeedTest.this);
		...
		findViewById(R.id.startButton).setVisibility(View.GONE);
		findViewById(R.id.TestInProgressButton).setVisibility(View.VISIBLE);
		Intent intent = new Intent(MMCIntentHandlerOld.SPEED_TEST);
		intent.putExtra(SpeedTestTrigger.EXTRA_SPEED_TRIGGER, 0);
		intent.putExtra(SpeedTestTrigger.EXTRA_UPDATE_UI, true);
		sendBroadcast(intent);
...
//		}
	...
	private void getCarrierSpeed(final Location location) {
			...
			mGetCarrierSpeedTask = new AsyncTask<Void, Void, JSONObject>() {
				...
				protected JSONObject doInBackground(Void... params) {
					try {
						...
						if (speedTier == 0)
							speedTier = MMCPhoneStateListenerOld.getNetworkGeneration(networkType);
						MMCLogger.logToFile(MMCLogger.Level.DEBUG, "SpeedTest", "getCarrierSpeed", "request stats for lat,lng " + latitude + "," + longitude);
						...
						MMCLogger.logToFile(MMCLogger.Level.ERROR, "SpeedTest", "getCarrierSpeed", "error", e);
		...
		/*
			...
			mCarrierAvg.setVisibility(View.INVISIBLE);
	...
	private void displayStats (JSONObject stats)
	{
		try
		{
			...
			MMCLogger.logToFile(MMCLogger.Level.ERROR, "SpeedTest", "getCarrierSpeed.onPostExecute", "error", e);
	...
	private void setTechnologyIcon() {
		...
		if(activeconn == 0) {
			...
			int networkGeneration = MMCPhoneStateListenerOld.getNetworkGeneration(networkType); 
	...
	*/
	...
	class ResultsListener extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if(intent.getAction().equals(MMCIntentHandlerOld.ACTION_SPEEDTEST_RESULT)) {
				if(intent.hasExtra(MMCIntentHandlerOld.EXTRA_LATENCY)) {
					int latencyProgress = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_LATENCY_PROGRESS, 0);
					if(latencyProgress < 100) {
						...
						long latency = intent.getLongExtra(MMCIntentHandlerOld.EXTRA_LATENCY, 0);
						if (latency == 0)
							mLatency.setText("-");
						else
				...
				if(intent.hasExtra(MMCIntentHandlerOld.EXTRA_DOWNLOAD_SPEED)) {
					int downSpeed = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_DOWNLOAD_SPEED, 0);
					setGaugeDownloadSpeed(downSpeed);
					
					int downProgress = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_DOWNLOAD_PROGRESS, 0);
					if(downProgress < 100) {
				...
				if(intent.hasExtra(MMCIntentHandlerOld.EXTRA_UPLOAD_SPEED)) {
					int upSpeed = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_UPLOAD_SPEED, 0);
					setGaugeUploadSpeed(upSpeed);
					
					int uploadProgress = intent.getIntExtra(MMCIntentHandlerOld.EXTRA_UPLOAD_PROGRESS, 0);
			...
			else if (intent.getAction().equals(MMCIntentHandlerOld.ACTION_SPEEDTEST_COMPLETE))
			{
			...
			else if(intent.getAction().equals(MMCIntentHandlerOld.ACTION_SPEEDTEST_ERROR) || intent.getAction().equals(MMCIntentHandlerOld.ACTION_SPEEDTEST_CANCELLED)) {
				if (intent.getAction().equals(MMCIntentHandlerOld.ACTION_SPEEDTEST_ERROR) )
				{
			...
			else if(intent.getAction().equals(MMCIntentHandlerOld.ACTION_SIGNAL_STRENGTH_UPDATE) || intent.getAction().equals(MMCIntentHandlerOld.ACTION_CONNECTION_UPDATE)) {
				setTechnologyIcon ();
			}
		}
	}
...
//			if (location == null)
//				return true;
//			if (location.getAccuracy() < 400)  // We'll settle for 400 meters for statistics
//			{
...
//				{
//					//if (bFreshGps == false && (mGetCarrierSpeedTask == null || mGetCarrierSpeedTask.getStatus() != AsyncTask.Status.RUNNING))
//					{
//						getCarrierSpeed(location);
//						statsLocation = location;
//						nearLocation = location;
