...
public class MMCActivity extends FragmentActivity {
    ...
    public static void customizeTitleBar(Context screen, View view, int mmc_title, int custom_title) {
        ...
        View dashBackground = (View) view.findViewById(R.id.dashBackground);
        if (customBackgroundColor.length() > 1) {
            scolor = Integer.parseInt(customBackgroundColor, 16) + (0xff000000);
            View centralBackground = (View) view.findViewById(R.id.centralBackground);
            if (centralBackground != null)
                centralBackground.setBackgroundColor(scolor);
        }
        if (customDashBackgroundColor.length() > 1) {
            scolor = Integer.parseInt(customDashBackgroundColor, 16) + (0xff000000);
            if (dashBackground != null)
                dashBackground.setBackgroundColor(scolor);
        } else if (customBackground == 1) {
            if (dashBackground != null)
                dashBackground.setBackgroundResource(R.drawable.bg_custom);
        } else if (dashBackground != null)
            dashBackground.setBackgroundResource(R.drawable.bg_gradient);
